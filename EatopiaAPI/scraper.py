...
for link in restaurant_links:
    ...
    if images is not None:
        ...
        for image in images:
            ...
            restaurant_image_info = {
                ...
                "Restaurant_id": counter_restaurants,
            }
    ...
    if time_span:
        restaurant_opening_hours = time_span.text.replace(" Open now", '').replace(
            " Closed now", '').replace(" See all hours", '').replace(":&nbsp;", '')[2:]
        if len(restaurant_opening_hours) > 20:
            restaurant_opening_hours = restaurant_opening_hours[:18] + \
                " and " + restaurant_opening_hours[18:]
    ...
    if cuisine_span:
        cuisine_list = [cuisine_elem.text.replace('$', '').replace('-', '').replace(' ', '').replace(
            ',', '') for cuisine_elem in cuisine_span.find_all("a", {"class": "dlMOJ"}) if len(cuisine_elem.text) > 3]
        cuisine_list = list(
            filter(lambda cuisine: cuisine != '', cuisine_list))
